    ...
    while True:
        ...
        if button == 'Open PGN File':
            ...
            if filename is not None and filename:
                ...
                i = 0
                board = copy.deepcopy(initial_board)
                window.find_element('_movelist_').Update(value='')
        ...
        if type(button) is tuple and moves is not None and i < len(moves):
            ...
            move_from = move.move.from_square   # parse the move-from and move-to squares
            move_to = move.move.to_square
            row, col = move_from // 8, move_from % 8
            piece = board[row][col]             # get the move-from piece
            button = window.find_element(key=(row, col))
